...
class MultiFileWriter:
    ...
    def upload_to_gcp(self):
        ...
        blob = self.bucket.blob(f"postings_gcp/{file_name}")  # Todo change path
    ...
    def read(self, locs, n_bytes, index_type):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                blob = my_bucket.get_blob(f"postings_gcp_{index_type}/" + f_name)
...
class InvertedIndex:
    ...
    def posting_lists_iter(self, index_type):
        """ A generator that reads one posting list from disk and yields
            a (word:str, [(doc_id:int, tf:int), ...]) tuple.
        """
        with closing(MultiFileReader("205963135")) as reader: # TODO change from hard coded
            for w, locs in self.posting_locs.items():
                b = reader.read(locs, self.df[w] * TUPLE_SIZE, index_type)
                posting_list = []
    ...
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name):
        ...
        blob_posting_locs = bucket.blob(f"postings_gcp/{bucket_id}_posting_locs.pickle")  # todo change the path
