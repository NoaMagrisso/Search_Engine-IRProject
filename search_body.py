...
def read_posting_list(inverted, w):
    ...
    with closing(MultiFileReader(bucket_name)) as reader:
        ...
        b = reader.read(locs, inverted.df[w] * TUPLE_SIZE, "body")
        posting_list = []
...
def get_candidate_documents_and_scores(query_to_search, index, words, pls, DL):
    ...
    for term in np.unique(query_to_search):
        if term in words:
            ...
            normlized_tfidf = [(doc_id, (freq / DL[doc_id]) * math.log(N / index.df[term], 10)) for doc_id, freq in list_of_doc]
...
def cosine_similarity(D, Q, doc_vector_length_dict):
    """
    ...
    for doc_id, row in D.iterrows():
        top = np.dot(row, Q)
        di_size = doc_vector_length_dict[doc_id]
...
def get_topN_score_for_queries(query_to_search, index, DL, doc_vector_length_dict, N=3):
    """
    ...
    """
    # YOUR CODE HERE
    w_pls_dict = get_posting_gen(index, query_to_search)  # TODO change
    words = tuple(w_pls_dict.keys())
    pls = tuple(w_pls_dict.values())
    D = generate_document_tfidf_matrix(query_to_search, index, words, pls, DL)
    Q = generate_query_tfidf_vector(query_to_search, index, DL)
    cosine_sim_dict = cosine_similarity(D, Q, doc_vector_length_dict)
